...
// Memo v2. Memo v1 (Memo1UhkJRfHyvLMcVucJwxXeuD728EqVDDwQDxFMMG) no longer
// exists on devnet; using it fails with ProgramAccountNotFound.
...
function buildS3Prefix(commitShort) {
  ...
  const explicit = (process.env.EVIDENCE_BUNDLE || "").trim();
  const root = explicit
    ? explicit.replace(/\/+$/, "")
    : `${(process.env.GITHUB_REPOSITORY || "repo").split("/").pop()}/ci`;

  return `${root}/${datePart}/${timePart}-${commitShort}`;
}

function resolveArtifactFiles(artifactDir) {
  const manifest = (process.env.ARTIFACT_FILES || "").trim();
  if (manifest) {
    return manifest
      .split(",")
      .map((s) => s.trim())
      .filter((f) => f && existsSync(`${artifactDir}/${f}`));
  }
  if (!existsSync(artifactDir)) return [];
  return readdirSync(artifactDir).filter((f) => {
    try {
      return statSync(`${artifactDir}/${f}`).isFile();
    } catch {
      return false;
    }
  });
...
async function zipArtifacts(artifactDir, outputDir) {
  const present = resolveArtifactFiles(artifactDir);
  if (present.length === 0) {
    throw new Error(
      `No CI artifact files found in ${artifactDir}. ` +
        `Set ARTIFACT_FILES or populate ${artifactDir}/ before running attest.`,
    );
  }
